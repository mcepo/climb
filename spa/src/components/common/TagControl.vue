...
export default Vue.extend({
  ...
  methods: {
    createTag () {
      this.$store.dispatch('auth/authorize', { item: this.item, callback: () => { drawingService.createTag(this.type, this.item) } })
    },
    editTag () {
      this.$store.dispatch('auth/authorize', { item: this.item, callback: () => { drawingService.editTag(this.type, this.item) } })
    }
  }
})
</script>
